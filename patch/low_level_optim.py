...
class LowLevelZeroOptimizer(OptimizerWrapper):
    ...
    def _run_reduction(self):
        for bucket_store in self.pg_to_bucket_store.values():
            ...
            if torch.isnan(flat_grads).any():
                raise RuntimeError(f"rank {dist.get_rank()} got nan on flat_grads")
            ...
            with get_accelerator().stream(stream):
                ...
                if not self._partition_grads:
                    ...
                    if torch.isnan(received_grad).any():
                        raise RuntimeError(f"rank {dist.get_rank()} got nan on received_grad")
