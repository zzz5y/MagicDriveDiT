        ...
        for i, batch in pbar:
            ...
            for ns in range(num_sample):
                ...
                save_data = {
                    ...
                    "_model_args": _model_args,  # 保存 _model_args
                    #B, T, NC
                    "B" :B,
                    "T": T,
                    "NC":NC,
                ...
                # 保存目录路径
                pt_dir = "./pt"
                mkdir_or_exist(pt_dir)  # 确保目录存在

                # 确保路径创建成功
                if os.path.exists(pt_dir):
                    print(f"Directory {pt_dir} exists.")
                else:
                    print(f"Failed to create {pt_dir}.")
                # 保存到文件
                torch.save(save_data, os.path.join(pt_dir, "inference_data.pt"))
