...
from magicdrivedit.mmdet_plugin.core.bbox.box_np_ops import points_cam2img
from magicdrivedit.mmdet_plugin.datasets import NuScenesDataset
...
def get_ego_velocity(nusc, sample, keyframe):
    """ego velocity for ego car.
    Samples are either 12Hz or 2Hz. There is no need to check max time diff.

    Args:
        sample (dict): sample dict.
        keyframe (bool): whether it is a keyframe sample. For non-keyframe
            samples, `sample` is its next sample.
    """
    current = sample
    if keyframe:
        # on keyframe, we consider both prev and next (if any).
        has_prev = current['prev'] != ''
        has_next = current['next'] != ''
    else:
        # for non-keyframes, we consider current and prev.
        has_prev = True
        has_next = False
    ...
    def _get_ego_pose(sample):
        sd_rec = nusc.get("sample_data", sample["data"]["LIDAR_TOP"])
        pose_record = nusc.get("ego_pose", sd_rec["ego_pose_token"])
        return pose_record

    pos_last = np.array(_get_ego_pose(last)['translation'])
    pos_first = np.array(_get_ego_pose(first)['translation'])
    pos_diff = pos_last - pos_first

    q_last = Quaternion(_get_ego_pose(last)['rotation'])
    q_first = Quaternion(_get_ego_pose(first)['rotation'])
    q_diff = (q_last * q_first.inverse).normalised
    q_diff = q_diff.angle * q_diff.axis  # angular velocity, 3-dim vector
    ...
    return (pos_diff / time_diff, q_diff / time_diff)


def get_box_ids(nusc, sample, keyframe=False):
    if keyframe:
        lidar_token = sample["data"]["LIDAR_TOP"]
        _, boxes, _ = nusc.get_sample_data(lidar_token)
        instance_tokens = []
        for box in boxes:
            record = nusc.get('sample_annotation', box.token)
...
def _sample_data_transform(
    ...
    if keyframe:
        ...
        if _CHECK_LIDAR:
            try:
                ...
                print(f"file {lidar_path} not found, we will skip file check!")
                _CHECK_LIDAR = False
    else:
        lidar_path = None
        boxes = []
    ego_velocity = get_ego_velocity(nusc, sample, keyframe)
    ...
    info['gt_box_ids'] = get_box_ids(nusc, sample, keyframe)
    info["gt_names"] = names
